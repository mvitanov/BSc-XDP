...
#include "xdpsock.h"
#include "xdp-stack/ipstack.h"
#include "xdp-stack/c_queue.h"
...
static void print_benchmark(bool running)
{
	...
	if (opt_bench == BENCH_RXDROP)
		...
		bench_str = "l2fwd";
	else if (opt_bench == BENCH_NETSTACK)
		bench_str = "ustack";
...
static void usage(const char *prog)
{
	const char *str =
		...
		"  -u, --ustack		Pass to usersapce stack\n"
		"  -d, --debug		Debug ustack logging and tracing\n"
...
static void parse_command_line(int argc, char **argv)
{
	...
	for (;;) {
		c = getopt_long(argc, argv, "rtli:q:psSNn:czud", long_options,
				&option_index);
		if (c == -1)
			break;
...
static void netstack(struct xdpsock *xsk)
{
	for (;;) {
		struct xdp_desc descs[BATCH_SIZE];
		unsigned int rcvd, i;
		...
		for (i = 0; i < rcvd; i++) {
			...
			//copy the packet and send it to the queue packets
			//memcpy(copy, pkt, descs[i].len);
...
static void netstack_all(void)
{
	pthread_t stack_thr;
	if (pthread_create(&stack_thr, NULL,
                       start_stack, NULL) != 0) {
	...
	if (pthread_join(stack_thr, NULL) != 0) {
